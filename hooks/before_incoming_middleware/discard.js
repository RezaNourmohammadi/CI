  ...
  const domyjob = async (bp, event) => {
    ...
    if (event.state.session.templateValid) {
      if (event.payload.text == '---discard---') {
        ...
        bp.logger.error(template_id)
        let discard_response = await axios.post(
          formsHost + '/api/botpress/user/agreements/' + template_id + '/remove_draft',
          null,
          {
            headers: {
              sessionid: sessionId,
        ...
        ////////////
        bp.logger.error(discard_response)
